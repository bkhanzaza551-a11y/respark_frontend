        ...
        <div className="panel-card">
          <form onSubmit={submit} style={{ display: "grid", gap: 12 }}>
            <div className="form-grid">
              <CustomDropdown value={form.customerId} onChange={(event) => setForm((current) => ({ ...current, customerId: event.target.value }))}>
                <option value="">Select customer</option>
                {context.customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}
              </CustomDropdown>
              <CustomDropdown value={form.bookingChannel} onChange={(event) => setForm((current) => ({ ...current, bookingChannel: event.target.value }))}>
            ...
            <div className="list-stack">
              ...
              {form.items.map((item, index) => (
                <div key={`appointment-edit-${index}`} className="list-item">
                  <div className="form-grid">
                    <CustomDropdown value={item.serviceId} onChange={(event) => updateItem(index, { serviceId: event.target.value })}>
