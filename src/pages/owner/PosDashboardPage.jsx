...
export default function PosDashboardPage() {
      ...
      {detail ? (
        <div className="premium-modal-overlay" onClick={closeDetail} style={{ zIndex: 9999, background: "rgba(0,0,0,0.6)" }}>
          <div className="premium-modal-content pos-dashboard-detail-modal" onClick={(event) => event.stopPropagation()}>
            <div className="pos-detail-header-strip" style={{ position: 'relative', minHeight: '36px' }}>
              ...
              <button type="button" onClick={closeDetail} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: '50%', cursor: 'pointer', padding: '6px', color: '#0f172a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={18} /></button>
            </div>
                ...
                <div className="pos-main">
                  ...
                  <div className="cart-table-body">
                    {form.items.map((item, index) => {
                      ...
                      return (
                        <div key={item.id || `${item.serviceId || item.productId || "item"}-${index}`} className="cart-table-row">
                          <div>
                              ...
                              <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 8, alignItems: "center" }}>
                                ...
                                {item.consumables?.length ? item.consumables.map((entry, cidx) => (
                                  <div key={`extra-${cidx}`} style={{ display: "flex", alignItems: "center", gap: 6, background: "#fafafa", padding: "3px 10px", borderRadius: 16, border: "1px solid #e5e7eb" }}>
                                    <span style={{ fontSize: 11, color: "#4b5563", fontWeight: 600 }}>{entry.name}</span>
                                    <span style={{ fontSize: 11, fontWeight: 700, color: "#111827" }}>{entry.qty}</span>
                                    <span style={{ fontSize: 10, fontWeight: 600, color: "#9ca3af", textTransform: "lowercase" }}>{entry.unit || 'pcs'}</span>
                                  </div>
                                )) : null}
                              </div>
                            )}
                          ...
                          <div style={{ display: "flex", alignItems: "center", gap: 4, flexWrap: "nowrap", justifyContent: "flex-end" }}>
                            <button type="button" title="Split Service" onClick={() => setStatus({ error: "", success: "Split service workflow is reserved for the next pass." })} style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--accent, #3b82f6)", padding: "2px" }} disabled={!isEditing}><ScissorsLineDashed size={16} /></button>
                            <button type="button" title="Service Reminder" onClick={() => openReminderModal(index)} style={{ background: "transparent", border: "none", cursor: "pointer", color: item.serviceReminder?.date ? "#16a34a" : "#111827", padding: "2px" }} disabled={!isEditing}><Clock3 size={16} /></button>
                            ...
                            }} style={{ background: "transparent", border: "none", cursor: "pointer", color: item.complimentary ? "#16a34a" : "#3b82f6", padding: "2px" }} disabled={!isEditing}><Gift size={16} /></button>
                            <button type="button" title="Add Consumable Items For Service" onClick={() => openConsumableModal(index)} style={{ background: "transparent", border: "none", cursor: "pointer", color: item.consumables?.length ? "#16a34a" : "#3b82f6", padding: "2px" }} disabled={!isEditing}><TicketPercent size={16} /></button>
                            {isEditing ? <button type="button" title="Remove Item" onClick={() => removeItem(index)} style={{ background: "transparent", border: "none", cursor: "pointer", color: "#ef4444", padding: "2px" }}><Trash2 size={16} /></button> : null}
                          </div>
                        </div>
                      );
                    })}
  ...
  {/* ======= FULL ADD PACKAGE MODAL ======= */}
      {showPkgModal && (
        <div className="pos-modal-overlay" style={{ zIndex: 10010 }} onClick={() => setShowPkgModal(false)}>
          <div className="pos-modal-container pos-modal-container-wide" onClick={e => e.stopPropagation()}>
            ...
            <div className="pos-modal-body">
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:6, background: "#f8fafc", padding: "16px", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
                  <label style={{ fontWeight:700, color:"#334155", fontSize:"0.88rem" }}>Add Services to Package</label>
                  <div style={{ position:"relative" }}>
                    <input placeholder="Search Service By Category Or Name..." value={pkgServiceSearch} onChange={e => setPkgServiceSearch(e.target.value)} style={{ width:"100%", padding:"10px 14px", paddingRight:36, border:"1px solid #cbd5e1", borderRadius:8, fontSize:"0.9rem", boxSizing:"border-box", outline: "none", transition: "border-color 0.2s" }} onFocus={e => e.target.style.borderColor="#3b82f6"} onBlur={e => e.target.style.borderColor="#cbd5e1"} />
            ...
            <div className="pos-modal-footer">
              <button onClick={() => setShowPkgModal(false)} style={{ padding:"10px 24px", background:"#fff", border:"1px solid #cbd5e1", borderRadius:8, fontWeight:700, cursor:"pointer", color:"#475569", transition: "all 0.2s" }} onMouseEnter={e => { e.currentTarget.style.background="#f1f5f9"; e.currentTarget.style.borderColor="#94a3b8"; }} onMouseLeave={e => { e.currentTarget.style.background="#fff"; e.currentTarget.style.borderColor="#cbd5e1"; }}>Cancel</button>
              <button onClick={handleAddPkgToCart} disabled={!pkgModalPkg || !pkgDraft.staffId} style={{ padding:"10px 24px", background:"#2563eb", color:"#fff", border:"none", borderRadius:8, fontWeight:800, cursor:(pkgModalPkg && pkgDraft.staffId)?"pointer":"not-allowed", opacity:(pkgModalPkg && pkgDraft.staffId)?1:0.5, boxShadow: "0 4px 6px -1px rgba(37, 99, 235, 0.2)", transition: "all 0.2s" }} onMouseEnter={e => { if(pkgModalPkg && pkgDraft.staffId) { e.currentTarget.style.background="#1d4ed8"; e.currentTarget.style.boxShadow="0 6px 8px -1px rgba(37, 99, 235, 0.3)"; } }} onMouseLeave={e => { if(pkgModalPkg && pkgDraft.staffId) { e.currentTarget.style.background="#2563eb"; e.currentTarget.style.boxShadow="0 4px 6px -1px rgba(37, 99, 235, 0.2)"; } }}>Add Package</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showMemModal && (
        <div className="pos-modal-overlay" style={{ zIndex: 10010 }} onClick={() => setShowMemModal(false)}>
