              ...
              <div style={{ padding: "12px 0", borderTop: "1px solid #f1f5f9" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 1 }}>Add consumable products used by this service</div>
                  </div>
                ...
                {(() => {
                  const consumables = srvForm.consumables || [];
                  return consumables.map((c, idx) => (
                      <div key={idx} style={{ background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 8, padding: "10px 12px", marginBottom: 8 }}>
                        <div style={{ display: "flex", gap: 8, alignItems: "flex-end" }}>
                          <div style={{ flex: 2, minWidth: 0 }}>
                            <CustomDropdown className="hub-input" value={c.productId} onChange={e => {
                              const ni = [...consumables];
                              const pr = products.find(p => p.id === e.target.value);
                              ni[idx] = {...ni[idx], productId: e.target.value, productName: pr?.name || ''};
                ...
                {!srvForm.consumables.length && <p style={{ margin: 0, fontSize: 12, color: "#94a3b8", fontStyle: "italic" }}>No consumables added</p>}
              </div>
