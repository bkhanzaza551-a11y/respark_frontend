...
if (!content.includes('import ToggleSwitch from')) {
...
const targetModalRegex = /\{showCouponModal && \(\s*<div style=\{\{ position: 'fixed', bottom: 0, left: 0, width: '100%', height: '100%', background: '#64748b', display: 'flex', flexDirection: 'column', padding: 24, boxSizing: 'border-box' \}\}>[\s\S]*?(?=\{showGiftCardModal && \()/;
...
if (content.match(targetModalRegex)) {
  content = content.replace(targetModalRegex, replacementModal);
  fs.writeFileSync('src/pages/owner/CouponsPage.jsx', content);
  console.log('Successfully refactored Coupon modal');
} else {
